...
export const GET = async (_: NextRequest, { params }: { params: tParams }) => {
  ...
  const result = await authenticatedBackendFetch(`/recurrent-transaction/${id}`);
  if (!result.ok) {
...
export const DELETE = async (_: NextRequest, { params }: { params: tParams }) => {
  ...
  const result = await authenticatedBackendFetch(`/recurrent-transaction/${id}`, { method: 'DELETE' });
  if (!result.ok) {
    return result.response;
  }
